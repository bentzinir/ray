...
class DynamicEnsembleTFPolicy(TFPolicy):
    ...
    def __init__(self,
        ...
        if action_sampler_fn:
            ...
            # action_dists = [dist_class(dist_input, self.model) for dist_input in dist_inputs]

            # Using exploration to get final action (e.g. via sampling).
            # sampled_actions = [None] * len(action_dists)
            # sampled_actions_logp = [None] * len(action_dists)
            # for aidx, action_dist in enumerate(action_dists):
            #     sampled_actions[aidx], sampled_actions_logp[aidx] = self.exploration.get_exploration_action(
